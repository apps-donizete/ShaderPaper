...
// --- Helper functions ---
vec3 mod289(vec3 x) {
    return x - floor(x * 0.00346020761) * 289.0; // 1/289 precomputed
}
vec4 mod289(vec4 x) {
...
// --- Original 3D simplex noise (ES2 compatible) ---
float snoise(vec3 v) {
    const vec2 C = vec2(0.16666667, 0.33333333); // 1/6, 1/3
...
void main() {
    ...
    vec2 light1 = vec2(sin(iTime*1.2+50.0)*1.0 + cos(iTime*0.4+10.0)*0.6,
